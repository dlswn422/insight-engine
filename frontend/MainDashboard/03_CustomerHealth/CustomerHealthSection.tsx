...
export default function CustomerHealthSection() {
  ...
  const orderTop5 = useMemo(() => {
    return [...items].sort((a, b) => a.order_trend - b.order_trend).slice(0, 5);
  }, [items]);
  ...
  return (
    <section className="section active" id="section-customer">
      <div className="section-header">
        ...
        <p>주요 고객사 건강도 및 파생 리스크 지표 모니터링</p>
      </div>
          ...
          <div className="customer-health-charts-row">
            <div className="chart-card customer-chart-card">
              <div className="chart-header">
                ...
                <span className="metric-badge metric-badge-derived">파생 지표</span>
              </div>
              <div className="customer-chart-canvas-wrap">
                <canvas ref={radarCanvasRef}></canvas>
              </div>
              <div className="customer-chart-note">
                최근 신호와 점수 기반으로 구성한 파생 분석 결과입니다.
              </div>
            </div>
            ...
            <div className="chart-card customer-chart-card">
              <div className="chart-header">
                ...
                <span className="metric-badge metric-badge-ai">AI 추정</span>
              </div>
              <div className="customer-chart-canvas-wrap">
                <canvas ref={barCanvasRef}></canvas>
              </div>
              <div className="customer-chart-note">
                실제 발주 원장 합계가 아닌 위험/기회 점수 기반 추정 변동치입니다.
              </div>
            </div>
          </div>
        </>
