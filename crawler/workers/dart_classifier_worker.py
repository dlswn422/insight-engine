...
# 한 번 실행 시 DB에서 가져올 PENDING 공시의 최대 건수입니다.
...
def get_pending_disclosures(limit: int = BATCH_SIZE) -> list[dict]:
    ...
    result = (
        ...
        .select("rcept_no, report_nm, source_role")  # 분류에 필요한 최소 컬럼만 조회
...
def classify_one(disclosure: dict) -> str:
    ...
    처리 순서:
        1순위) EXCLUDE_KEYWORDS에 해당하면 → SKIPPED (분석 제외)
        2순위) TARGET_KEYWORDS에 해당하면  → READY_FOR_LLM (LLM 분석 대기열에 투입)
        3순위) 어느 쪽도 해당 없으면       → UNMATCHED (분석 생략)

    반환값:
        "SKIPPED"       - EXCLUDE_KEYWORDS 해당
        "READY_FOR_LLM" - TARGET_KEYWORDS 해당
        "UNMATCHED"     - 해당 키워드 없음
    """
    ...
    # ── 1순위: 행정 공시 제외 (주주총회, 의결권 등) ──────────────
    for kw in EXCLUDE_KEYWORDS:
        if kw in clean:
            print(f"  ⏭️  건너뜀 {tag}: [{report_nm}] → 제외 키워드: '{kw}'")
            update_status(rcept_no, "SKIPPED")
            return "SKIPPED"

    # ── 2순위: 타겟 키워드 포함 → LLM 분석 대기열 투입 ──────────
    for kw in TARGET_KEYWORDS:
        if kw in clean:
            print(f"  🎯 LLM 분석 대기 {tag}: [{report_nm}] → 타겟 키워드: '{kw}'")
            update_status(rcept_no, "READY_FOR_LLM")
            return "READY_FOR_LLM"

    # ── 3순위: 어느 키워드도 해당 없음 ────────────────────────────
    print(f"  ❓ 미매칭 {tag}: [{report_nm}]")
...
def run() -> None:
    ...
    ※ LLM 호출 없음 — 빠르고 가볍습니다.
    ※ 이 워커 실행 후 dart_llm_worker.py를 실행하면 LLM 분석이 이어집니다.
    """
    print("=" * 60)
    print("[dart_classifier_worker] 키워드 분류 워커 시작")
    print(f"  배제 키워드: {len(EXCLUDE_KEYWORDS)}개  /  타겟 키워드: {len(TARGET_KEYWORDS)}개")
    print("=" * 60)

    total     = 0   # 전체 처리 건수
    skipped   = 0   # SKIPPED 건수
    ready     = 0   # READY_FOR_LLM 건수 (다음 워커 대상)
    unmatched = 0   # UNMATCHED 건수
    batch_num = 0   # 배치 반복 횟수

    # PENDING이 없을 때까지 배치를 반복합니다.
    while True:
        disclosures = get_pending_disclosures()
        if not disclosures:
            if batch_num == 0:
                print("[dart_classifier_worker] PENDING 공시가 없습니다. 종료합니다.")
            break

        batch_num += 1
        total     += len(disclosures)
        print(f"\n[배치 {batch_num}] 분류 대상: {len(disclosures)}건  (누적: {total}건)\n")
    ...
    if batch_num > 0:
        print(f"  총 처리 건수  : {total}건")
        print(f"  건너뜀        : {skipped}건")
        print(f"  LLM 분석 대기 : {ready}건  ← dart_llm_worker.py 실행 시 처리됩니다")
        print(f"  미매칭        : {unmatched}건")
    print("=" * 60)
